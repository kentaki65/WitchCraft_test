	...
	[{
		...
		onPlayerClickUp: (id, alt, x, y, z, block, target) => {
			...
			if (s >= 500) {
				...
				y += 1.5
				const f = api.getPlayerFacingInfo(id),
					[dx, dy, dz] = f?f.dir : [0, 0, 1]
				api.playParticleEffect({
					...
					texture: `glint`,
					manualEmitCount: Math.min(20, 3 + Math.floor(s / 500) * 2),
					minEmitPower: 1,
					maxEmitPower: 2,
					minLifeTime: s * 0.0003,
					maxLifeTime: s * 0.0003,
					minSize: 0.4,
					maxSize: 0.5,
					gravity: [0, 0, 0],
					blendMode: 1,
				...
				for (let d = 1; d <= points; d++) {
					iceShootIds[id].push(setTimeout(() => {
						let px = x + dx * d;
						let py = y + dy * d;
						let pz = z + dz * d;
						...
						for (const i of api.getEntitiesInRect([px - 1.5, py - 2, pz - 1.5], [
							px + 1.5, py + 2, pz + 1.5
						])) {
							if (i === id) continue;
							try {
								api.applyEffect(i, `Frozen`, s, {
									displayName: `Iced`,
									initiatorId: id
								})
							} catch (e) { }
							api.attemptApplyDamage({
								eId: id,
								hitEId: i,
								attemptedDmgAmt: Math.max(1, damage - d),
								withItem: `Iceball`
							})
						}
					}, d * 100))
		...
		onPlayerSelectInventorySlot: (id, slot) => {
			...
			if (api.getHeldItem(id)?.name === `Draugr Rod`) {
				api.setClientOption(id, `touchscreenActionButton`, [{
					icon: `Draugr Rod`
				}])
			} else {
		...
		onPlayerSwapInvenSlots: (id, i, j) => {
			...
			api.setClientOption(id, `runningSpeed`, 7);
			api.setClientOption(id, `walkingSpeed`, 4);
			const item = api.getItemSlot(id, j),
				n = item?.name?? "Dirt";
			if (itemData[n] && 0 <= i && 9 >= i) {
				api.setSelectedInventorySlotI(id, i);
				changeCall(id, j);
				api.setClientOption(id, `touchscreenActionButton`, [{
					icon: n
				}])
		...
		onPlayerDropItem: (id, x, y, z, item, amount, slot) => {
			if (iCharging[id]) {
				delete iCharging[id];
				api.removeMiddleScreenBar(id)
			}
			api.setClientOption(id, `runningSpeed`, 7);
			api.setClientOption(id, `walkingSpeed`, 4);
			if (api.getSelectedInventorySlotI(id) !== slot && api.getHeldItem(id)?.name ===
				`Draugr Rod`) {
				api.setClientOption(id, `touchscreenActionButton`, [{
					icon: `Draugr Rod`
				}])
		...
		tick: (id) => {
			...
			let c = [`white`, `lime`, `lime`, `lime`, `yellow`, `yellow`, `orange`,
				`orange`, `red`, `red`
			][Math.min(8, Math.floor(s / 500))]
		...
		onPlayerClickUp: (id, alt, x, y, z, block, target) => {
			...
			if (s >= 2000) {
				...
				for (let i = 1; i <= 10; i++) {
					...
					let ty = y + dir[1] * i
					let tz = z + dir[2] * i
					for (const e of api.getEntitiesInRect([tx - 4, ty - 4, tz - 4], [tx + 4,
					ty + 4, tz + 4
					])) {
						if (e === id || hit[e]) continue
						hit[e] = true
						try {
							api.applyEffect(e, `Frozen`, power * 250, {
								initiatorId: id
							})
							api.applyEffect(e, `Slowness`, power * 250 * 2, {
								initiatorId: id
							})
						} catch (e) { }
					}
			...
			if (touchDown) {
				...
				if (s >= 2000) {
					...
					for (let i = 1; i <= 10; i++) {
						...
						let ty = y + dir[1] * i
						let tz = z + dir[2] * i
						for (const e of api.getEntitiesInRect([tx - 4, ty - 4, tz - 4], [tx +
							4, ty + 4, tz + 4
						])) {
							if (e === id || hit[e]) continue
							hit[e] = true
		...
		tick: (id) => {
			...
			let s = api.now() - iCharging[id]
			api.shakePlayerCamera(id, s / 100000, 1000 / 20)
			let c = [`white`, `white`, `white`, `white`, `lime`, `yellow`, `orange`,
				`orange`, `red`, `red`
			][Math.min(8, Math.floor(s / 500))]
			api.sendFlyingMiddleMessage(id, [{
				icon: `fa-solid fa-crosshairs`,
